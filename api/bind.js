...
export default async function handler(req, res) {
  ...
  try {
    ...
    // 1. 嚴格檢查 Email 是否存在於教職員名冊中 (不分大小寫與前後空格)
    ...
    if (searchError || !existingStaff) {
      await recordAuditLog({
        ...
        actor_role: 'system',
        action: 'BIND_ACCOUNT_FAILED',
        target_table: 'staff',
        details: { email: cleanEmail, reason: 'EMAIL_NOT_FOUND', searchError: searchError?.message }
      });

      return res.status(400).json({ error: '找不到此 Email。請確認您輸入的信箱是否與學校白名單建檔相符（請檢查大小寫或是否遺漏 .tw 等後綴）。' });
    }
    ...
    if (isSuperAdmin(existingStaff)) {
      await recordAuditLog({
        ...
        action: 'BIND_ACCOUNT_FAILED',
        target_table: 'staff',
        details: { email: cleanEmail, reason: 'SUPER_ADMIN_BLOCKED' }
      });
    ...
    const isMandatoryMode = process.env.REQUIRE_INVITE_CODE === 'true';

    // 若系統啟用了安全強制查核 (REQUIRE_INVITE_CODE=true)，或者該教職員帳號有指定專屬授權碼
    if (isMandatoryMode || expectedCode) {
      ...
      if (!expectedCode && !isExplicitlyApproved) {
        await recordAuditLog({
          ...
          action: 'BIND_ACCOUNT_FAILED',
          target_table: 'staff',
          details: { email: cleanEmail, reason: 'INVITE_CODE_REQUIRED_BUT_MISSING' }
        });
      ...
      if (expectedCode && (!providedCode || providedCode !== expectedCode)) {
        await recordAuditLog({
          ...
          action: 'BIND_ACCOUNT_FAILED',
          target_table: 'staff',
          details: { email: cleanEmail, reason: 'INVITE_CODE_MISMATCH' }
        });
    ...
    if (existingStaff.line_uid && existingStaff.line_uid !== verifiedUid) {
      await recordAuditLog({
        ...
        action: 'BIND_ACCOUNT_FAILED',
        target_table: 'staff',
        details: { email: cleanEmail, reason: 'ALREADY_BOUND_TO_ANOTHER_UID', existing_uid: existingStaff.line_uid }
      });

      return res.status(409).json({ error: '此信箱已綁定其他 LINE 帳號。為保障帳號安全，若需換綁請洽系統管理員於名冊中點選「解綁」後再重試。' });
    }
    ...
    await recordAuditLog({
      actor_uid: verifiedUid,
      actor_role: 'system',
      action: 'BIND_ACCOUNT_SUCCESS',
      target_table: 'staff',
      details: { email: cleanEmail, previous_uid: existingStaff.line_uid }
    });
